...
# Copy flux-pro directory to root project
COPY --link flux-pro ./flux-pro

# Install flux-pro dependencies
RUN cd flux-pro && composer install \
    --no-dev \
    --no-interaction \
    --no-autoloader \
    --no-ansi \
...
# Copy main project composer files
COPY --link composer.* ./
